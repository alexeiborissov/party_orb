...
astore(where(astore lt 0))=0
 ...
 IF ((n_elements(ypb[2,*]) gt 2) AND (n_elements(yp[2,*]) gt 2)) THEN BEGIN
 ...
 oAR3 = obj_new('IDLgrSymbol', oModel)
 IF Obj_Valid(oAR3) THEN oAR3->SetProperty, Size=mysymsize
 xplot3d, ts[*,0]/arbscl,ts[*,1]/arbscl, ts[*,2]/arbscl, COLOR=nflcol, sym=oAR3, /OVERPLOT 
 ...
 XPLOT3D, ts[*,0]/lscl, ts[*,1]/lscl, ts[*,2]/lscl, COLOR=[0,0,0], NAME='end', SYMBOL=oSymbol2, THICK=2, /OVERPLOT
 
 XPLOT3D, [-1,1], [0,0], [0,0], COLOR=[0,0,0], NAME='end', THICK=3, /OVERPLOT, linestyle=2
 
 fieldflag=0
FOR i=2,npts DO BEGIN
 IF ((i MOD 131) eq 0) THEN fieldflag=1 ELSE fieldflag=0
...
ENDFOR
 XPLOT3D, [0,0], [0,0], [0,0], COLOR=[255,255,255], NAME='end', /OVERPLOT, filename=newoutdir+string(e,format='("ip",i2.2,".png")')
ENDFOR
;cgwindow
...
;title='Particle peak energy gain (eV)', /vertical, /ylog
 cgColorbar, Divisions=3, Minor=10, Format='(e8.1)', Ticklen=-0.25, /window,range=[10^t1,10^t2], /vertical, /ylog

whatcolouris20eV=alog10(20)/alog10(1000)*256.
end
