...
particletrack, 1, floc=nom, xyzt, lcol=[0,0,0], lscl=1e6, zscl=1e6, /mm, myxyrange=[-100.0,100.0], myzrange=[-60,60], /rel, ax=-60, az=40, filewidth=800
stop
for i=2,npts DO BEGIN
 particletrack, i, /op, floc=nom, xyzt, lcol=[r(astore(i-1)),g(astore(i-1)),b(astore(i-1))], zscl=1e6, /mm, /rel, myxyrange=[-100.0,100.0], myzrange=[-60,60]
endfor


;STOP

iniorbplot, 1, op=0, floc=nom, ocol=[r(astore(0)),g(astore(0)),b(astore(0))], zscl=1e6, /km, /rel, myxyrange=[-0.4,0.4], myzrange=[-20,20], ax=-77, az=30, filewidth=800
;iniorbplot, 1, op=0, floc=nom, ocol=[0,0,0], zscl=1e6, /km, /rel, myxyrange=[-0.4,0.4], myzrange=[-20,20], ax=-77, az=30, filewidth=800

for i=2,npts DO BEGIN
print, i, npts, format='(i4,"/",i4)'
 iniorbplot, i, /op, floc=nom, ocol=[r(astore(i-1)),g(astore(i-1)),b(astore(i-1))], zscl=1e6, /km, /rel
