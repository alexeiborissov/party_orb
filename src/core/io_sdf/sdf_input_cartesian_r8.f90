...
CONTAINS
  ...
  SUBROUTINE read_1d_float_r8(h, variable, distribution, subarray)
	...
	!print*, 'does read_1d_float_r8 ever get called?'
  ...
  SUBROUTINE read_nd_float_r8(h, variable, distribution, subarray)
    ...
    CALL MPI_FILE_SET_VIEW(h%filehandle, h%current_location, MPI_BYTE, &
       ...
       !print*, 'nd: file_set_view done'

    CALL MPI_FILE_READ_ALL(h%filehandle, variable, 1, subarray, &
        MPI_STATUS_IGNORE, errcode)

       !print*, 'nd: file_read_all done'


    CALL MPI_FILE_SET_VIEW(h%filehandle, c_off0, MPI_BYTE, MPI_BYTE, 'native', &
        MPI_INFO_NULL, errcode)

       !print*, 'nd: file_set_view redone'
...
!	print*, 'does read_3d_variable_r8 ever get called?'
