function getdata, ii, n_c=n_c, wkdir=wkdir, muBsquared=muBsquared, fields=fields, $
...
IF keyword_set(ke) or keyword_set(all) THEN BEGIN
 ...
 e3=transpose(A(15,*))
 ek=transpose(A(16,*))
 thedata=create_struct(thedata,{e2:e2,e3:e3,ek:ek})
ENDIF
IF keyword_set(drifts) or keyword_set(all) THEN BEGIN
 Ue=transpose([[transpose(A(23,*))],[transpose(A(24,*))],[transpose(A(25,*))]]) 
; Ugb=transpose([[transpose(A(26,*))],[transpose(A(27,*))],[transpose(A(28,*))]])
; Uc=transpose([[transpose(A(29,*))],[transpose(A(30,*))],[transpose(A(31,*))]])
; thedata=create_struct(thedata,{Ue:Ue,Ugb:Ugb,Uc:Uc})
ENDIF
IF keyword_set(gyro) or keyword_set(all) THEN BEGIN
 ;rL=transpose(A(23,*))
 gyrofreq=transpose(A(29,*))
 gyroperiod=transpose(A(30,*)) 
 gyrorad=transpose(A(31,*))
