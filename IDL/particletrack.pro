PRO particletrack, ii, n_c=n_c, tscl=tscl, op=op, floc=floc, lscl=lscl, xyzt, symb=symb, lcol=lcol, zscl=zscl, bsymb=bsymb, mytitle=mytitle, me=me, km=km, Mm=Mm, myxrange=myxrange, myyrange=myyrange, myzrange=myzrange, rel=rel, myls=myls, myfline=myfline, ax=ax, az=az, filewidth=filewidth, frac=frac, newxyr=newxyr
;@getdata
...
IF n_elements(myxrange) eq 0 THEN myxrange=[-100,100]
IF n_elements(myyrange) eq 0 THEN myyrange=[-100,100]
...
IF n_elements(frac) 	eq 0  THEN frac=2.2
...
ENDIF
 ...
 ;oOrb = OBJ_NEW('orb', COLOR=[255, 255 ,255])	;white orb
 ;oOrb = OBJ_NEW('orb', COLOR=[255, 255 ,0]) 	;yellow orb
 ;oOrb = OBJ_NEW('orb', COLOR=[0, 255 ,0]) 
 ;oOrb2 = OBJ_NEW('orb', COLOR=[255, 0 ,0])
 ;oOrb3 = OBJ_NEW('orb', COLOR=[0, 0 ,255]) 	;blue orb
 ;oOrb2 = OBJ_NEW('orb', COLOR=[255, 51 ,255]) 	;pink orb
 ;oOrb2 = OBJ_NEW('orb', COLOR=[255, 102 ,255]) 	;lighter pink orb
 ;oOrb2 = OBJ_NEW('orb', COLOR=[0, 0 ,0])    	;black orb
 ;oOrb3 = OBJ_NEW('orb', COLOR=[0, 0 ,255]) 	;blue orb
 
 oOrb2 = OBJ_NEW('orb', COLOR=[51, 153 ,255]) 	;light green orb
 oOrb = OBJ_NEW('orb', COLOR=[102, 255 ,102]) 	;light green orb
 ...
 oOrb->Scale, frac*xyrat, frac*xyrat, zlen/xlen*xyrat*frac
 oOrb2->Scale, frac*xyrat, frac*xyrat, zlen/xlen*xyrat*frac
 ...
 oModel->add,symbol_obj(2,[0, 0 ,0],[0,0,0])	    ; black tetrahedon
 ;oModel->add,symbol_obj(2,[128, 0 ,255],[0,0,0])	    ; purple tetrahedon
 ;oModel->add,symbol_obj(2,[128, 0 ,128],[0,0,0])	    ; blue tetrahedon
 ...
 IF Obj_Valid(oTet) THEN oTet->SetProperty, Size=[frac*xyrat, frac*xyrat, zlen/xlen*xyrat*frac]*9.9
 ;xobjview,oTet
 ;STOP
 ...
 IF keyword_set(myfline) THEN BEGIN ; if field lines are required, draw them first (in black)
   ...
   IF (op EQ 0) THEN BEGIN
    xplot3d, yp(0,*)/lscl, yp(1,*)/lscl, yp(2,*)/zscl, COLOR=flc, xrange=myxrange, yrange=myyrange, $
  ...
  ENDIF ELSE BEGIN ; else just draw the field lines (check whether overplot is on though)
   IF (op EQ 0) THEN BEGIN
    ;xplot3d, x/lscl, y/lscl, z/zscl, xrange=myxyrange, yrange=myxyrange+0.5, $
    xplot3d, x/lscl, y/lscl, z/zscl, xrange=myxrange, yrange=myyrange, $
