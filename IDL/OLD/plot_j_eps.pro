...
xmaxs=localmax(x)
; oplot,x[xmaxs],y[xmaxs],psym=2
print,getcolor('red',2)
...
device,filename='j.eps',/encapsulated,/color,bits=8
plot,t[xmaxs],j,xtitle='time - right side (x>0) bounce points only',yrange=[0,max(j)*1.1],ytitle='J (needs multiplied by mass and possibly tscl)',title='variation between min/max and average j: '+string(j_var,format='(f10.5)')+'%'
plot,t[xmaxs],j,/noerase,position=[.5,.2,.9,.6],/normal
; write_png,"j.png",tvrd()
device,/close

device,filename='length.eps',/encapsulated,/color,bits=8
...
device,filename='j_points.eps',/encapsulated,/color,bits=8

plot,x,y,xtitle="x / 10Mm",ytitle="y / 10 Mm",title="Points used as start/end of integration"
oplot,x[xmaxs],y[xmaxs],color=getcolor('red',110),psym=2
