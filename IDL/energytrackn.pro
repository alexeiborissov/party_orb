...
particletrack, 1, floc=nom, xyzt, lcol=[r(cstore(0)),g(cstore(0)),b(cstore(0))], zscl=1e6, /mm, myxyrange=[-100,100], myzrange=[-60,60], /bsymb
for i=2,npts DO BEGIN
 particletrack, i, /op, floc=nom, xyzt, lcol=[r(cstore(i-1)),g(cstore(i-1)),b(cstore(i-1))], zscl=1e6, /mm, myxyrange=[-100,100], myzrange=[-60,60]
endfor
...
;STOP
iniorbplot, 1, op=0, floc=nom, ocol=[r(cstore(0)),g(cstore(0)),b(cstore(0))], zscl=1e6, /km, myxyrange=[-0.4,0.4], myzrange=[-20,20]

for i=2,npts DO BEGIN
print, i, npts, format='(i4,"/",i4)'
 iniorbplot, i, /op, floc=nom, ocol=[r(cstore(i-1)),g(cstore(i-1)),b(cstore(i-1))], zscl=1e6, /km
endfor
